...
class Seria(object):
    ...
    def stream(self, stream):
        if not (hasattr(stream, 'read') or hasattr(stream, 'getvalue')):
            raise Seria.Error("Stream must be file-like object")
        self._stream = stream
        self.formats = get_formats(self.stream)
    ...
    def dump(self, fmt, *args, **kwargs):
        if fmt == 'yaml':
            ...
            raise Seria.Error("Requested format '%s' is not a supported format" % fmt)
    ...
    def load(self):
        ...
        # No need for JSON.load since yaml is a superset of json
            ...
            raise Seria.Error("Input does not appear to be a supported format")